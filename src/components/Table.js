...
const ItemReturner = ({ value, data, onEdit, setOpen, setId }) => {
  ...
  } else if (value.is_category_image) {
    return (
      <td>
        <img
          className="product_image"
          src={data[value.value]}
          height="24" //TODO: ratio here..
          width="24"
        />
      </td>
    );
  } else if (value.is_title) {
    return <td>{data.title_info[0]?.name?.tr}</td>;
  } else if (value.is_type) {
    return <td>{data.type_info[0]?.name?.tr}</td>;
  } else if (value.is_category) {
    return <td>{data.category_info[0]?.name?.tr}</td>;
  ...
  } else if (value.is_city) {
    let formatted_city = getCityFromId(data[value.value]);
